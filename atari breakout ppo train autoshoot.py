        ...
        for t in range(max_timesteps):
            ...
            if done:
                ...
                if total_reward > best_reward:
                    ...
                    torch.save(agent.policy.state_dict(), 'ppo_breakout_autoshoot_best.pth')

                if (episode + 1) % 10 == 0:
                    avg_reward = np.mean(reward_history[-10:])
                    print(f"Episode {episode + 1}, Reward: {total_reward}, Avg: {avg_reward}")
                if (episode + 1) % 1000 == 0:
                    torch.save(agent.policy.state_dict(), f'ppo_breakout_autoshoot_{episode + 1}.pth')
                break

    env.close()
    torch.save(agent.policy.state_dict(), "ppo_breakout_autoshoot.pth")
